# CDM_BrFull (CDM Full Bridge Measurement)

The CDM_BrFull instruction is used to make a full bridge measurement on aCDM **Note:** . For additional information on controller area network peripherals and Campbell modules, see [CPI and CDM](https://www.campbellsci.com/news-cpi-cdm).
...
(CDMType,CPIAddress,Dest,Reps,Range,DiffChan,ExChan,MeasPEx,ExmV,RevEx,RevDiff,SettlingTime,fN1,Mult, Offset)

The following program provides an example of measuring a Druck CS420 pressure transducer using a CDM_BrFull measurement.
...
'measurementLvl_ft:
CDM_BrFull(VOLT108,1,Lvl_ft,1,mv200C,1,X1,1,2500,True,True,0,60,2.3067,0)
...
For the CDM_BrFull instruction, measurements are made on consecutive channels. If the Reps parameter is greater than 1, the Dest parameter must be a variable array.
