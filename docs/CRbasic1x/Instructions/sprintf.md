...
The following program writes readable, customized logs to a text file called CustomLog.Txt on the dataloggerUSRdrive. The log file contains the battery voltage with 2 decimal places and the panel temp with 1 decimal place.
...
OpenFile1 = FileOpen ("USR:CustomLog.txt","a",-1)
...
The following program writes a daily log file to the dataloggerUSRdrive with a dynamic filename based on date. In this example, replace "Logged Data Line" with what you would like to write to the log file.
...
Sprintf(FileName, "USR:Log_%04u%02u%02u.txt", DLTime(1), DLTime(2), DLTime(3))
