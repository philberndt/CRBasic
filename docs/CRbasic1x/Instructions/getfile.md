...
The following example program shows the use of the GetFile instruction. The datalogger is connected to another datalogger via theCOMC1(C1/C2) serial port. The [GetVariables](getvariables.md) instruction is used to get the name of the last file stored by the TableFile instruction in the remote datalogger. The datalogger then compares this last file name to see if it has changed since the last GetVariables attempt. If the file has changed the datalogger retrieves the new file.
...
BeginProg
SerialOpen (COMC1,57600,0,0,10000)
Scan (1,Sec,3,0)
GetVariables (ResultGetVar,COMC1,0,3000,0000,0,"Public","LastFileName",LastFileName,1)
If LastFileName<>CompLastFileName
GetFile(ResultGetFile,COMC1,0,3000,0000,0,LastFileName,LastFileName)
