...
The following example program sends a file named DatCR1000X.dat that is stored on the datalogger'sCRDdrive to another datalogger with a PakBus ID of 1086 when Flag(1) is high. A PC running LoggerNet (PakBus ID 4094) is used to route the packet from one datalogger to the other. Note that in this instance, the PakBus ports in LoggerNet must be bridged or the SendFile will fail.
...
TableFile ("CRD:DatCR1000X",8,2,10,0,0,OutStat,LastFileName)
Sample (1,TCTemp,FP2)
EndTable
