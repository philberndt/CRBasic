...
function FMCInit() {
                ...
                if (!FMCIsWebHelp() || MCGlobals.RootFrame != null) {
                  ...
                  if (pulsePath) {
                    ...
                    gServiceClient.GetPulseStreamID(
                      ...
                      function (streamID) {
                        FMCSetPulseFrameSrc(
                          gServiceClient.PulseServer + "streams/" + streamID + "/activities?frame=stream"
                        );
                      },
                      null
                    );
  ...
  if (MadCap.Slideshow) MadCap.Slideshow.Init();
...
function FMCInitBodyCommentsFrame() {
  if (MCGlobals.BodyCommentsFrame == null && FMCIsLiveHelpEnabled()) {
    var iframe = document.getElementById("topiccomments");
    if (iframe) MCGlobals.BodyCommentsFrame = iframe.contentWindow;
  }
}
...
function FMCCheckForBookmark() {
  ...
  if (bookmark) {
    ...
    if (FMCIsHtmlHelp())
      //Bug 86027 - Skip the rest for chm files. It's an issue with html help topics containing the X-UA-Compatible meta tag
      return;
...
function Body_OnMessage(e) {
  ...
  if (messageType == "request") {
    ...
    if (message == "set-topic-comments-height") {
      ...
      responseData[responseData.length] = FMCGetMCAttribute(
        MCGlobals.BodyFrame.document.documentElement,
        "MadCap:browseSequencePath"
      );
      ...
      if (dlg) CMCDialog.Close(dlg.RootEl);

      handled = true;
      needsCallbackFired = false;
    } else if (message == "reload-pulse") {
      var commentsFrame = frames["topiccomments"];
      if (commentsFrame) FMCPostMessageRequest(commentsFrame, "reload", null, null, null, true);
      ...
      if (gServiceClient) gServiceClient.PulseUserGuid = dataValues[0];
...
if (gRuntimeFileType == "Topic") {
  ...
  if (window.postMessage != null) {
    if (window.addEventListener) window.addEventListener("message", Body_OnMessage, false);
    else if (window.attachEvent) window.attachEvent("onmessage", Body_OnMessage);
  }
}
