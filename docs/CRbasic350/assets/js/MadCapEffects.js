    ...
    if (!span) {
      ...
      span.appendChild(
        document.createTextNode(
          "Topic popups can not be displayed when Insert Mark of the Web is enabled in the target."
        )
      );
...
function FMCSetPopupSize(popupNode) {
  ...
  if (
    (popupWidth != "auto" && !String.IsNullOrEmpty(popupWidth)) ||
    (popupHeight != "auto" && !String.IsNullOrEmpty(popupHeight))
  ) {
  ...
  while (true) {
    ...
    if (
      FMCGetScrollHeight(popupFrame.window) > FMCGetClientHeight(popupFrame.window, false) ||
...
function FMCPopupThumbnail_Onclick(e, node) {
  ...
  function OnDocumentKeyDown(e) {
    ...
    if (e.keyCode != 27 && e.keyCode != 13)
      // Escape and enter key support to close thumbnail popup
      return;
...
function FMCResizeSlideshow(el, open, expandHeadHeight) {
  if (el) {
    ...
    if ($slideshow) {
      ...
      if (open) newHeight = $slideshow.height() + Math.max(newHeight, $el.outerHeight()) - expandHeadHeight;
      else newHeight = $slideshow.height() - Math.max(newHeight, $el.outerHeight()) + expandHeadHeight;
