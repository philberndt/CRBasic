...
The ModbusServer instruction configures a datalogger as a ModbusRTU
...
This instruction sets up a Modbus server device. As a Modbus server, the datalogger will respond to data request from a Modbus client over serial and TCP/IP. Supported Modbus functions are 01, 02, 03, 04, 05, 15, and 16. See the [Modbus Client()](modbusclient.md) help for details on these functions.
...
- A datalogger acting as a Modbus server returns a Modbus exception code 01 (Illegal Function) if a query is invalid, and an exception code of 02 (Illegal data address) if data values beyond the size of an array are requested. If a client requests variables that are not defined, the VarOutofBound field is incremented in the datalogger's Status table.
