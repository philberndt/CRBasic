...
window.onload = function () {
  if (document.location.href.Contains("frame=stream")) return;
...
function FMCIsTopicPopup(win) {
  if (window.name.indexOf("-html5") != 0 || window.name == "pulse") return false;
...
function FMCGetBodyHref() {
  ...
  var bodyHref =
    bodyLocation.protocol +
    (!FMCIsHtmlHelp() && !FMCIsWebHelpAIR() ? "//" : "") +
    bodyLocation.host +
    bodyLocation.pathname +
    bodyLocation.hash;
...
function FMCPreloadImage(imgPath) {
  if (
    !(FMCIsEclipseHelp() || FMCIsWebHelp()) ||
    (!FMCInPreviewMode() && !FMCGetAttributeBool(document.documentElement, "PreloadImages", false))
  ) {
    return;
  }
...
function FMCRegisterCallback(frameName, eventType, CallbackFunc, callbackArgs) {
  ...
  function FMCCheckTOCInitialized() {
    ...
    FMCPostMessageRequest(
      MCGlobals.NavigationFrame.frames["toc"],
      "gInit",
      null,
      function (data) {
        var isInit = FMCStringToBool(data[0]);
  ...
  function FMCCheckSearchInitialized() {
    ...
    FMCPostMessageRequest(
      MCGlobals.NavigationFrame.frames["search"],
      "gInit",
      null,
      function (data) {
        var isInit = FMCStringToBool(data[0]);
  ...
  if (frameName == "TOC") {
    if (eventType == MCEventType.OnLoad) {
      func = FMCCheckTOCLoaded;
    } else if (eventType == MCEventType.OnInit) {
      func = FMCCheckTOCInitialized;
    } else if (eventType == MCEventType.OnReady) {
      func = FMCCheckTOCReady;
    }
  } else if (frameName == "Toolbar") {
    if (eventType == MCEventType.OnLoad) {
      func = FMCCheckToolbarLoaded;
    } else if (eventType == MCEventType.OnInit) {
      func = FMCCheckToolbarInitialized;
    } else if (eventType == MCEventType.OnReady) {
      func = FMCCheckToolbarReady;
    }
  } else if (frameName == "BodyComments") {
    if (eventType == MCEventType.OnLoad) {
      func = FMCCheckBodyCommentsLoaded;
    } else if (eventType == MCEventType.OnInit) {
      func = FMCCheckBodyCommentsInitialized;
    } else if (eventType == MCEventType.OnReady) {
      func = FMCCheckBodyCommentsReady;
    }
  } else if (frameName == "Persistence") {
    if (eventType == MCEventType.OnLoad) {
      func = FMCCheckPersistenceLoaded;
    } else if (eventType == MCEventType.OnInit) {
      func = FMCCheckPersistenceInitialized;
    } else if (eventType == MCEventType.OnReady) {
      func = FMCCheckPersistenceReady;
    }
  } else if (frameName == "Search") {
    if (eventType == MCEventType.OnLoad) {
      func = FMCCheckSearchLoaded;
    } else if (eventType == MCEventType.OnInit) {
      func = FMCCheckSearchInitialized;
    } else if (eventType == MCEventType.OnReady) {
      func = FMCCheckSearchReady;
    }
  } else if (frameName == "MCGlobals") {
    if (eventType == MCEventType.OnLoad) {
      func = FMCCheckMCGlobalsLoaded;
    } else if (eventType == MCEventType.OnInit) {
      func = FMCCheckMCGlobalsInitialized;
    } else if (eventType == MCEventType.OnReady) {
      func = FMCCheckMCGlobalsReady;
    }
  } else if (frameName == "Navigation") {
    if (eventType == MCEventType.OnLoad) {
      func = FMCCheckNavigationLoaded;
    } else if (eventType == MCEventType.OnInit) {
      func = FMCCheckNavigationInitialized;
    } else if (eventType == MCEventType.OnReady) {
      func = FMCCheckNavigationReady;
    }
  } else if (frameName == "Body") {
    if (eventType == MCEventType.OnLoad) {
      func = FMCCheckBodyLoaded;
    } else if (eventType == MCEventType.OnInit) {
      func = FMCCheckBodyInitialized;
    } else if (eventType == MCEventType.OnReady) {
      func = FMCCheckBodyReady;
    }
  } else if (frameName == "Root") {
    if (eventType == MCEventType.OnLoad) {
      func = FMCCheckRootLoaded;
    } else if (eventType == MCEventType.OnInit) {
      func = FMCCheckRootInitialized;
    } else if (eventType == MCEventType.OnReady) {
      func = FMCCheckRootReady;
    }
  }
...
function FMCSetCookie(name, value, days) {
  if (!FMCIsValidCookie(name)) return;

  if (!FMCIsValidCookie(value)) return;
...
function FMCSetupButtonFromStylesheet(
  tr,
  styleName,
  styleClassName,
  defaultOutPath,
...
CMCXmlParser.GetXmlDoc = function (xmlFile, async, LoadFunc, args, loadContextObj) {
  ...
  function LoadScript(success) {
    ...
    CMCXmlParser._LoadingFilesPathMap.ForEach(function (key, value) {
      ...
      if (loadingFileUrl.Name == fileName && loadingFileUrl.FullPath != jsFileUrl.FullPath) {
        ...
        CMCXmlParser.GetXmlDoc(
          loadingFileUrl.FullPath,
          loadInfo.async,
          loadInfo.LoadFunc,
          loadInfo.args,
          loadInfo.loadContextObj
        );
  ...
  if (FMCIsLocal() && !FMCIsDotNetHelp() && !FMCIsHtmlHelp()) {
    // DotNetHelp and HtmlHelp do not generate js files
    var xmlFileUrl = new CMCUrl(xmlFile);
    var jsFileUrl = xmlFileUrl.ToExtension("js");
    var fileName = jsFileUrl.Name;

    CMCXmlParser._LoadingFilesPathMap.Add(jsFileUrl.FullPath, {
      async: async,
      LoadFunc: LoadFunc,
      args: args,
      loadContextObj: loadContextObj,
    });
...
var CMCFlareStylesheet = new (function () {
  ...
  this.Init = function (OnCompleteFunc) {
    FMCGetStylesheet(function (xmlDoc) {
      if (xmlDoc != null) mRootNode = xmlDoc.documentElement;
  ...
  this.SetImageFromStylesheet = function (
    img,
    styleName,
    styleClassName,
    propertyName,
...
CMCDictionary.prototype.GetItem = function (key) {
  if (this.mIgnoreCase) key = key.toLowerCase();
...
function FMCGetByAttribute(node, args) {
  ...
  try {
    if (
      node.nodeType == 1 &&
      (FMCGetMCAttribute(node, attribute) == value || (value == "*" && FMCGetMCAttribute(node, attribute)))
    ) {
      nodes[nodes.length] = node;
    }
  } catch (err) {
...
function MakeButton(td, title, outImagePath, overImagePath, selectedImagePath, width, height, text) {
  ...
  if (text != null) buttonEl.appendChild(document.createTextNode(text));
...
var MCFader = new (function () {
  ...
  this.FadeIn = function (node, startOpacity, endOpacity, nodeBG, startOpacityBG, endOpacityBG, handleClick) {
    ...
    function DoFadeIn() {
      if (!FMCIsInDom(node)) {
        // Node was already removed from the DOM.
...
var CMCDateTimeHelpers = new (function () {
  ...
  this.ToDurationString = function (fromDate, toDate) {
    ...
    if (hours < 1) return parseInt(minutes) + " minutes ago";
    if (days < 1) return parseInt(hours) + " hours ago";
    if (days < 30) return parseInt(days) + " days ago";

    var months = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"];
    var dateString = months[fromDate.getMonth()] + " " + fromDate.getDate();

    if (fromDate.getFullYear() != toDate.getFullYear()) dateString += ", " + fromDate.getFullYear();
...
var MCGlobals = new (function () {
  ...
  this.Init = function () {
    ...
    if (window.name == "bridge") {
      ...
      if (FMCIsWebHelp()) {
        if (window.name == "mctoolbar")
          // Toolbar in WebHelp
          InitToolbarWebHelp(); // Toolbar in WebHelp topic
        else InitToolbarWebHelpTopic();
      } // Toolbar in CHM
      else {
        InitToolbarCHM();
      }
      ...
      // Topic in WebHelp - window.name is "body" when viewed in frameset, but gRuntimeFileType might not be "Topic" if it's an external topic. gRuntimeFileType is "Topic" when viewed as standalone, but window.name won't be "body".
...
function FMCPostMessageRequest(win, message, data, callbackFunc, postMessageNotSupportedFunc, alwaysUsePostMessage) {
  ...
  if (FMCIsChromeLocal() || alwaysUsePostMessage) {
    ...
    win.postMessage(
      "request" + gMessageSeparator + message + gMessageSeparator + dataString + gMessageSeparator + gMessageID,
      "*"
    );
...
function FMCPostMessageResponse(win, message, data, messageID) {
  ...
  win.postMessage(
    "response" + gMessageSeparator + message + gMessageSeparator + dataString + gMessageSeparator + messageID,
    "*"
  );
